$(document).ready(function () {
    ...
    var selectRowFn = function (rowid, status, obj) {
        var rowData = $('#mctrHomeDeptToGrid').jqGrid('getRowData', rowid);
        rowObject = rowData;
    };
    ...
    mctrObj.CreateJqGrid('mctrHomeDeptToGrid', '/MctrCreateForm/getRgListHomeDeptToJSON', 'POST', mctrLineItem, columnNames, colModel, false, '', [], 10, '100%', 'desc', selectRowFn);

    $('#mctrHomeDeptToOkBtn').click(function () {
        $('#formOrToFlag').val('to');
        var rowId = $('#rowId').val();
        ...
        var dept_status = '';
        var homeBuglTo = $('#' + rowId + '_HOME_BUGL_TO').val();
        var workBuglTo = $('#' + rowId + '_WORK_BUGL_TO').val();
        var flagGlobal = false;
        //    var flagGlobalValues = (global_home_dept_to == '');

      //  var flagWorkDept = $('#' + rowId + '_HOME_LOC_TO').val() != '' && $('#' + rowId + '_WORK_DEPT_TO').val() == '' || work_loc_to == '' || work_pool_to == '' || $('#' + rowId + '_WORK_DEPT_TO').val() != home_dept_to || work_loc_to != home_loc_to || work_pool_to != home_pool_to;
        var flagWorkDept = rowObject.ACCTG_LOC_CD7 != '' && (work_dept_to == '' || work_loc_to == '' || work_pool_to == '' || work_dept_to != rowObject.DEPTID || work_loc_to != rowObject.ACCTG_LOC_CD7 || work_pool_to != rowObject.ALLOW_POOL_CD7);
        //var flagWorkDept=home_dept_to != '' && home_dept_to == work_dept_to && home_loc_to != '' && home_loc_to == work_loc_to;
        if ((!$.isEmptyObject(rowObject)) || (home_dept_to == '' && global_home_dept_to != '')) {
            ...
            if (home_dept_to != '') {
                ...
                if ((work_dept_to == '' && work_loc_to == '') || (work_dept_to == home_dept_to && work_loc_to == '')) {
                ...
                else if (rowObject.ACCTG_LOC_CD7 != '' && (work_dept_to == '' || work_loc_to == '' || work_pool_to == '' || work_dept_to != rowObject.DEPTID || work_loc_to != rowObject.ACCTG_LOC_CD7 || work_pool_to != rowObject.ALLOW_POOL_CD7)) {
                    bootbox.confirm("Do you want to override Work Department/Location using Home Department/Location selection? Click OK button to accept.", function (result) {
                        if (result) {
                            $('#' + rowId + '_WORK_DEPT_TO').val(rowObject.DEPTID);
                            $('#' + rowId + '_WORK_DEPT_TO').attr('value', rowObject.DEPTID);
                         //   $('#' + rowId + '_HOME_LOC_TO').val(rowObject.DEPTID);
                         //   $('#' + rowId + '_HOME_LOC_TO').attr('value', rowObject.DEPTID);
                        //    $('#' + rowId + '_HOME_POOL_TO').val(rowObject.DEPTID);
                        //    $('#' + rowId + '_HOME_POOL_TO').attr('value', rowObject.DEPTID);

                            $('#' + rowId + '_WORK_LOC_TO ').val(rowObject.ACCTG_LOC_CD7);
                            $('#' + rowId + '_WORK_LOC_TO ').attr('value', rowObject.ACCTG_LOC_CD7);
                            $('#' + rowId + '_WORK_POOL_TO ').val(rowObject.ALLOW_POOL_CD7);
                            $('#' + rowId + '_WORK_POOL_TO ').attr('value', rowObject.ALLOW_POOL_CD7);
                           // $('#' + rowId + '_LABOR_RATE_CD7_TO ').val(rowObject.ACCTG_LOC_CD7);
                          //  $('#' + rowId + '_LABOR_RATE_CD7_TO ').attr('value', rowObject.ACCTG_LOC_CD7);
    ...
    $('#mctrHomeDeptcancelBtn').click(function () {
        $('#' + rowId + '_HOME_DEPT_TO').val($('#' + rowId + '_HOME_DEPT_TO').attr('value'));
    });
    mctrObj.searchGrid('mctrHomeDeptToGrid');
});