$(document).ready(function () {
    ...
    var selectRowFn = function (rowid, status, obj) {
        var rowData = $('#ListWorkDeptFromGrid').jqGrid('getRowData', rowid);
        rowObject = rowData;
    };
    ...
    mctrObj.CreateJqGrid('ListWorkDeptFromGrid', '/MctrCreateForm/getRgListWorkDeptFromLOVJson', 'POST', mctrLineItem, columnNames, colModel, false, '', [], 10, '100%', 'desc', selectRowFn);
    $("#ListWorkDeptFromGrid").navGrid('#pager', { add: false, edit: false, del: false, search: false, refresh: true }, {}, {}, {}, {});
    $('#ListWorkDeptFromOkBtn').click(function () {
        var rowId = $('#rowId').val();
        ...
        var global_rsc_from = $('#' + rowId + '_RSC_FROM').attr('value');// == "" ? $('#' + rowId + '_RSC_FROM').val() : $('#' + rowId + '_RSC_FROM').attr('value');
    ...
    $('#ListWorkDeptcancelBtn').click(function () {
        ...
        $('#' + rowId + '_WORK_DEPT_FROM').val($('#' + rowId + '_WORK_DEPT_FROM').attr('value'));
    });
    mctrObj.searchGrid('ListWorkDeptFromGrid');
});